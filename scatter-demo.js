...
document.querySelector('#demo-grid').innerHTML=designs.map((d,i)=>`<article class="demo" id="demo-${i+1}"><div class="demo-head"><div><span class="demo-index">0${i+1}</span><h2>${d.name}</h2><p>${d.copy}</p></div><span class="tag">${d.tag}</span></div><svg class="plot" viewBox="0 0 620 215" role="img" aria-label="${d.name} scatter plot">${axes()}${papers.sort((a,b)=>r(b)-r(a)).map(p=>mark(p,d.kind)).join('')}</svg></article>`).join('');
